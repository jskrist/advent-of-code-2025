...
pub fn main(scenario: &String, input_root: String) {
    ...
    let mut results = vec![0u64; line_len];
    for line in lines {
        ...
        for m in matches.clone() {
            ...
            splitter_ind.push(i);
            // remove the index from tracked beams
            tach_ind.remove(&i);
        }
        for i in splitter_ind.iter() {
            // add the adjacent indices to be tracked
            let left_ind = i.saturating_sub(1);
            let right_ind = min(i + 1, line_len);
    ...
    println!("The tachyon beam split {} times", results.iter().sum::<u64>());
}
